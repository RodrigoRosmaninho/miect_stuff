...
PtABP ABPCreate (void)
{
	...
	PtABP ptree = (PtABP) malloc(sizeof(struct abp));
	if (ptree == NULL) { Error=NO_MEM; return NULL; }
	ptree->Size = 0;
	ptree->Root = NULL;
	Error = OK;
	return ptree;
...
{
	/* Insira o seu c�digo - Insert your code */
	if((*proot)->PtLeft != NULL) DestroyTree(&((*proot)->PtLeft));
	if((*proot)->PtRight != NULL) DestroyTree(&((*proot)->PtRight));
...
{
	/* Insira o seu c�digo usando a SearchTree - Insert your code using SearchTree */
	if (ptree == NULL) { Error = NO_ABP; return 0; }
	if (pelem == NULL) { Error = NULL_PTR; return 0; }
	if (ptree->Size == 0) { Error = ABP_EMPTY; return 0; }

	Error = OK;
	PtFraction res = SearchTree(ptree->Root, pelem);
	if(res == NULL) {
		Error = NO_ELEM;
		return 0;
	}
...
static PtFraction SearchTree (PtABPNode proot, PtFraction pelem)
{
	...
	if(FractionCompareTo(pelem, proot->PtElem) < 0) return SearchTree(proot->PtLeft, pelem);
...
void ABPInsert (PtABP ptree, PtFraction pelem)	/*  */
{
	...
	while(current != NULL)
	{
		if(FractionCompareTo(pelem, current->PtElem) < 0)
		{
			previous = current;
			current = current->PtLeft;
		}
		else if(FractionCompareTo(pelem, current->PtElem) > 0)
		{
			previous = current;
	...
	if(FractionCompareTo(previous->PtElem, pelem) > 0) previous->PtLeft = newNode;
	else previous->PtRight = newNode;
	ptree->Size++;
	Error = OK;
...
{
	/* Insira o seu c�digo usando MaxTree - Insert your code using MaxTree */
	if (ptree == NULL) { Error = NO_ABP; return NULL; }
	if (ptree->Size == 0) { Error = ABP_EMPTY; return NULL; }
...
PtABP ABPCopy (PtABP ptree)
{
	/* Insira o seu c�digo - Insert your code */
	if (ptree == NULL) { Error=NO_ABP; return NULL; }
	PtABP newTree = ABPCreate();
	PtStack stack = StackCreate(sizeof(PtABPNode));
	if (newTree == NULL || stack == NULL) { Error = NO_MEM; return NULL; }

	if(StackPush(stack, &(ptree->Root)) != OK) { Error=NO_MEM; StackDestroy(&stack); ABPDestroy(&newTree); return NULL; };
	...
	while(StackIsEmpty(stack) == 0)
	{
		...
		if(StackPop(stack, &pop) != OK) { StackDestroy(&stack); ABPDestroy(&newTree); return NULL; };
		pelem=FractionCopy(pop->PtElem);
		if (pelem == NULL) { FractionDestroy(&pelem); StackDestroy(&stack); ABPDestroy(&newTree); return NULL; }
		ABPInsert(newTree, pelem);

		if(pop->PtRight != NULL)
		{
			if(StackPush(stack, &(pop->PtRight)) != OK) { Error = NO_MEM; StackDestroy(&stack); ABPDestroy(&newTree); return NULL; }
		}

		if(pop->PtLeft != NULL)
		{
			if(StackPush(stack, &(pop->PtLeft)) != OK) { Error = NO_MEM; StackDestroy(&stack); ABPDestroy(&newTree); return NULL; }
		}
...
static PtQueue ABPFillQueue (PtABP ptree)
{
	/* Insira o seu c�digo - Insert your code */
	if(ptree == NULL) { Error = NO_ABP; return NULL; }
	PtQueue res = QueueCreate(sizeof(PtFraction*));
	PtQueue tmp = QueueCreate(sizeof(PtABPNode));
	if (res == NULL || tmp == NULL) { Error = NO_MEM; return NULL; }

	if(QueueEnqueue(tmp, &(ptree->Root)) != OK) { Error = NO_MEM; QueueDestroy(&res); QueueDestroy(&tmp); return NULL; }

	PtABPNode pop;
	while(!QueueIsEmpty(tmp)){
		if(QueueDequeue(tmp, &pop) != OK) { QueueDestroy(&tmp); QueueDestroy(&res); return NULL; };
		if(QueueEnqueue(res, &(pop->PtElem)) != OK) { Error = NO_MEM; QueueDestroy(&res); QueueDestroy(&tmp); return NULL; }

		if(pop->PtLeft != NULL)
		{
			if(QueueEnqueue(tmp, &(pop->PtLeft)) != OK) { Error = NO_MEM; QueueDestroy(&res); QueueDestroy(&tmp); return NULL; }
		}

		if(pop->PtRight != NULL)
		{
			if(QueueEnqueue(tmp, &(pop->PtRight)) != OK) { Error = NO_MEM; QueueDestroy(&res); QueueDestroy(&tmp); return NULL; }
		}
	}
...
void ABPReunion (PtABP ptree1, PtABP ptree2)
{
	...
	// NO_MEM vem do ABPFillQueue

	PtFraction fraction;
	while(!QueueIsEmpty(queue)){
		if (QueueDequeue(queue, &fraction) != OK) { QueueDestroy(&queue); return; }
		PtFraction aux = FractionCopy(fraction);
		if(aux == NULL) { QueueDestroy(&queue); return; }
		if(!ABPSearch(ptree1, fraction)) ABPInsert(ptree1, aux);
	}
...
void ABPDifference (PtABP ptree1, PtABP ptree2)
{
	...
	while(!QueueIsEmpty(queue)){
		if (QueueDequeue(queue, &fraction) != OK) { QueueDestroy(&queue); return; }
		if(ABPSearch(ptree1, fraction)) ABPDelete(ptree1, fraction);
	}
...
void ABPIntersection (PtABP ptree1, PtABP ptree2)
{
	...
	while(!QueueIsEmpty(queue)){
		if (QueueDequeue(queue, &fraction) != OK) { QueueDestroy(&queue); return; }
		if(!ABPSearch(ptree2, fraction)) ABPDelete(ptree1, fraction);
	}
