...
PtFraction FractionCreate (int pnum, int pden)	/* construtor inicializador - initializer constructor */
{
	...
	PtFraction f = CreateFraction();
	if(f == NULL) { Error = NO_MEM; return NULL;}

	Error = OK;
	FractionSet(f, pnum, pden);
	return f;
}
...
{
	if((pfrac1 == NULL) || (pfrac2 == NULL)) { Error = NO_FRACTION; return 0; }
	int num = (pfrac1 -> Num * pfrac2 -> Den) + (pfrac1 -> Den * pfrac2-> Num);
...
{
	if((pfrac1 == NULL) || (pfrac2 == NULL)) { Error = NO_FRACTION; return 0; }
	if(FractionIsZero(pfrac2)) { Error = ZERO_DIV; return NULL;}
	int num = pfrac1 -> Num * pfrac2 -> Den;
	int den = pfrac1 -> Den * pfrac2 -> Num;
	Error = OK;
    return FractionCreate(num, den);
}
...
int FractionEquals (PtFraction pfrac1, PtFraction pfrac2)
{
	if((pfrac1 == NULL) || (pfrac2 == NULL)) { Error = NO_FRACTION; return 0; }
	Error = OK;
	return pfrac1 -> Num == pfrac2 -> Num && pfrac1 -> Den == pfrac2 -> Den;
}
...
int FractionCompareTo (PtFraction pfrac1, PtFraction pfrac2)
{
	if((pfrac1 == NULL) || (pfrac2 == NULL)) { Error = NO_FRACTION; return 0; }
	Error = OK;
	return (pfrac1 -> Num * pfrac2 -> Den) - (pfrac1 -> Den * pfrac2-> Num);
}
...
int FractionIsProper (PtFraction pfrac)
{
	if(pfrac == NULL) { Error = NO_FRACTION; return 0; }
	Error = OK;
	return abs(pfrac -> Num) < pfrac -> Den;
}
